			...
			<div className="container">
				<div className="card">
					...
					<div className="flex flex-col items-center space-y-4">
						<label className="drop-zone" onDrop={handleDrop} onDragOver={preventDefault}>
							...
							{selectedFiles.length > 0 ? (
								<div>
									{selectedFiles.map((file, index) => (
										<div key={index} className="selected-file">
											{file.name}
											<button className="remove-file" onClick={() => handleDiscard(index)}>
												Remove
											</button>
										</div>
									))}
								...
								<span className="text-xl font-medium text-gray-500">Insert or Drop files here</span>
							)}
						</label>
